...
async def async_setup_entry(
    ...
    # Pull the dynamic data dictionary distributed by __init__.py
    config_data = hass.data[DOMAIN][entry.entry_id]
    coordinator = config_data["coordinator"]
    device_id = config_data["device_id"]

    # Generate and append the 4 sensor classes dynamically passing the device_id
    async_add_entities([
...
class UpsaiVoltageSensor(CoordinatorEntity, SensorEntity):
    ...
    def native_value(self):
        """Return the current value from coordinator cache safely."""
